...
@if ($assessment)
    ...
    @if (!empty($assessment->eltt3) && file_exists(public_path($assessment->eltt3)))
            <a href="{{ asset($assessment->eltt3) }}" target="_blank" class="view-file">Endorsement Letter To TESDA</a><br>
        @else
 
        @endif

    @if (!empty($assessment->rfftp3) && file_exists(public_path($assessment->rfftp3)))
            <a href="{{ asset($assessment->rfftp3) }}" target="_blank" class="view-file">Request Form For Test Package</a><br>
        @else
 
        @endif

    @if (!empty($assessment->oropfafns3) && file_exists(public_path($assessment->oropfafns3)))
            <a href="{{ asset($assessment->oropfafns3) }}" target="_blank" class="view-file">Official Receipt of Payment for Assessment for Non-Scholar</a><br>
        @else
  
        @endif
  
    @if (!empty($assessment->sopcctvr3) && file_exists(public_path($assessment->sopcctvr3)))
            <a href="{{ asset($assessment->sopcctvr3) }}" target="_blank" class="view-file">Submission of Previous CCTV Recordings</a><br>
        @else
